...
**State, dated 2026-09-14 at session close.** Branch `docs/rulesets-readme-no-longer-a-pair` is open
...
- exit 0, **680 passed across 33 files**, none skipped;
...
- **The README** now says:
  ...
  - what the check does without the ruleset, for a person's pull request, a Dependabot one and a
    fork's.
- **`tests/workflows/copilot-ruleset.test.ts`**, 14 tests:
  - Each thing the README says the check does is driven through `awaitRound`: it asks once, it
    decides once, a failed request keeps it waiting, a round has to land in time, and an unrecorded
    request lets a person's review land.
  ...
  - The stale claim, in its three written shapes, is refused in every tracked document, and four
...
- **`.portulan/gate-map.md`**: one phrase, "so the dependency is reviewable" → "so the ruleset is
  reviewable". It is the only change outside the README and its test.
...
- **Coupled to behaviour, not only a banned phrase.** A test that the README "no longer says" the
  ...
  across `documents()` per #73, and narrowed to the shapes they were written in once review showed a
  wider one reads true sentences.
- **What no test reaches is written down, not implied.** How soon the ruleset asks, and whether a
  ...
  the changelog both say so. The changelog said otherwise in the first pass, and that was the review's
  one required change.
...
- **A changelog entry, on #77's precedent over #59's.** Both are documentation corrections. #77
  ...
  a test too.
- **Shown to fail, not only to pass.** On the rebased head, each mutation was restored and the tree
  checked clean.
  - Against `main`'s README, 7 of 14 fail.
  ...
  - Each of these fails exactly the one test pinning it:
    ...
    - the stale claim restated in `CLAUDE.md`;
  ...
  - Two true sentences added to documents leave it green: one about drafts, one about
    `pull-requests: write`.

## What the reviews changed

Two reviews of `6dbe003`, and every finding was reproduced before it was acted on.

**An independent reviewer in its own worktree returned APPROVE-WITH-ADJUSTMENTS.** Its first run
stopped on a session rate limit after one message. It was resumed, against the rebased head, once
the limit reset.
...
**Copilot's round on the same head** said "Needs a closer look", in two suppressed comments with no
thread:

- **The gate map's "the dependency".** The reviewer had flagged it too. It is now fixed, and pinned as
  the scan's third shape.
...
**A slip of my own, caught by the guard.** The first pass at these fixes rewrote the README's
paragraph and first bullet but not the fork bullet. The updated test failed on the missing "lands
before the budget runs out" wording, and the command stopped before committing, as it was written to.
...
- **The gate map's "The platform floor" never names #58 or #64.** After describing the bot-author hole
  ...
  person's review. Both reviewers read it as implying that asking closes both holes. The README
  quotes only the sentence about the common path, which holds there.
- **`origin/main` moved under this session twice.**
  ...
  - Then #79 and #81, after #83 was open. That rebase needed a force-push with a lease and costs
