...
`claude/affectionate-almeida-876dbf`, cut from `main` at `c02b598` (#81) after a fetch confirmed
that was `origin/main`. `main` gained #82 while the session ran, and the branch was pushed one
behind it, so `origin/main` at `f6e1e9d` was merged in as `5b9d3d7` before the pull request, titled
`docs(portulan): the definition of done no longer says strict forces a rebase`, was opened. The
verify recipe at `5b9d3d7`, which differs from the head the pull request opened with only in this
file: exit 0, **701 passed across 32 files** with none skipped, 100% statements (263/263), branches
...
- **On the record, the clause was true when it was written.** #29 (`1aeba7a`, 2026-08-06) wrote
  ...
  provenance link. #37 updated the document that records the setting, not the one that cited it.
- **Did not reproduce: that the suite pins the gate map's `false`.** The brief said so, and
  ...
  places it says off: the full suite stayed green, 666 of 666 at `c02b598`, and the file was
  restored byte-identical. `tests/portulan/gate-policy.test.ts` checks that the map names the
  setting and carries the import warning, and reads no value. Nothing here rested on it — the live
  setting is the evidence — and it is not fixed here.
...
- **The other hits were read in context, and are true.** `gate-map.md` says `strict` *forced* a
  ...
  off. The handoffs of 2026-08-20 and 2026-08-22, and *the review an agent may not give* of
  2026-09-14, say it is off.
...
- **`main` merged in, not rebased onto.** The fetch that showed #82 ran in the same command as the
  first push, which did not wait on it, so the drift was read after the branch was published. A
  rebase would then have rewritten pushed commits; a merge did not, and no review round existed yet
  for either to invalidate.
...
**Recoverability.** Nothing partial: one commit to `dod.md`, this file, and a merge of `main`, on
one branch. No tag, release, publish or branch-protection change was touched. The measurement that
edited `gate-map.md` was undone before anything was committed, confirmed with `cmp`, and the tree
was clean after.
